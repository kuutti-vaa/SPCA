...
class sca_zscore(BaseEstimator):
    """
    Z score method for spatial class analysis (SCA)
    ...
    def fit(self, X, y = None):
        ...
        for parent_mol in self.uni_parents:
            ...
            if not parent_metabs or len(parent_metabs) < self.min_entity:
                ...
                class_mat = class_mat.T.values
                self.class_matrices.append(class_mat)
                zscores = stats.zscore(class_mat, axis=1)
                sum_zscore = np.sum(zscores, axis=0)
                pathway_act = sum_zscore / np.sqrt(class_mat.shape[0])
                self.fitted_models.append(pathway_act)
